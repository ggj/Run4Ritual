#include "redskullentity.h"
#include "entityfactory.h"
#include "../scene/gamescene.h"

ENTITY_CREATOR("RedSkull", RedSkullEntity)

RedSkullEntity::RedSkullEntity()
	: ItemEntity("RedSkull", "RedSkull")
{
}

void RedSkullEntity::OnCollision(const CollisionEvent &event)
{
	if (event.GetType() == CollisionEventType::OnEnter)
	{
			...
			(other != nullptr && other->GetClassName() == "Player4"))
		{
			...
			//Collect Item
			player->OnCollect(ItemTypes::RedSkull, this->iAmount);
